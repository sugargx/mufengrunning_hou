...
//        );
    ...
    public function index(Request $request)
    {
        ...
        $flag=($userInfo->Name!='');//标记是否已经完善姓名、年龄、家庭住址、性别、电话个人基本信息
            return ['registerFlag'=>$flag,'GroupID'=>$userInfo->GroupID,'Id'=>$userInfo->Id];
    }
    //获取用户基本信息，初始化程序
...
//        最后要把注释打开
        $Id=$request->Id;
        //管理员可以因为测试需求需要无限次打卡
        if($Id==3||$Id==20||$Id==4) return 'true';
...
//            $Beginy=date('y',$weekBegin);//年不要了
            ...
            $thisBegin=date('y-m-d',$weekBegin);
            $nextBegin=date('y-m-d',$weekBegin+60*60*24*7);
            $thisWeekRun=DB::table('run')->where('UserMainID',$ID)->whereDate('Date','>=',$thisBegin)->whereDate('Date','<',$nextBegin)->sum('Distance');
            $thisWeekRun=round($thisWeekRun ,2);
            array_push($weekSum,$thisWeekRun);
            /*周总跑量块结束*/
            $week=array();
...
//        dump($run);
        ...
        for($i=$RegisterDate;$i<time();$i+=60*60*24*32){
            ...
            /*this月总跑量块开始*/
            $thisMonthRun=DB::table('run')->where('UserMainID',$ID)->whereDate('Date','>=',date('y-m-d',$i))->whereDate('Date','<',date('y-m-d',$next))->sum('Distance');
            $thisMonthRun=round($thisMonthRun ,2);
            array_push($monthSum,$thisMonthRun);
            /*this月总跑量块结束*/
...
//            array_push($monthTime,$days);
    ...
    public function getMyRunList(Request $request){
        $Id=$request->Id;
        $runlist=DB::table('run')->where('UserMainID',$Id)->select('Id','Distance','Date')->get();
    ...
    public function runDetail(Request $request){
        ...
        $Run=DB::table('run')->where('Id',$Id)->first();
        ...
        $Run->TotalRun=DB::table('run')->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->sum('Distance');
        $Run->TotalRun=round($Run->TotalRun ,2);
        $Run->TimeAll=DB::table('run')->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->count();
        $Run->TotalPoints=DB::table('run')->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->sum('Point');
        $AdvImg=DB::table('advertise')->where('Id',1)->select('AdvTitle','AdvQRUrl')->first();
        return [$Run,$AdvImg];
    }
    //查看排名个人跑步图片
