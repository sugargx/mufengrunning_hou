...
//        最后要把注释打开
        $Id=$request->Id;
        //管理员可以因为测试需求需要无限次打卡
//        if($Id== 710||$Id== 708) return 'true';
        $lastTime=DB::table('run')->where('UserMainID',$Id)->orderBy('Id', 'desc')->first();
        if($lastTime){
            if((time()-strtotime($lastTime->Date))>=60*60*1)
                return 'true';
            else{
                $time=date('H:i',strtotime($lastTime->Date)+60*60*1);
                return $time;
            }
        }
        return 'true';
...
//            "page"             => "/pages/check/result?orderID=".$orderID,
    ...
    public function getZhangdi(){
        $res=DB::table('usermain')->where('name','张迪')->first();
        return [$res];
    }
}
