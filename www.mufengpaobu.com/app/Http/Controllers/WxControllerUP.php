            ...
            if($Group) {
                ...
                if($Status[$i]==1&&$GroupState[$i]==1){
                    ...
                    if($res){
                        ...
                        $storyNum = count(Story::where('groupId','=',$Group->Id)->where('state', '=', '0')->get());
                        if($storyNum>0){
                            $Group->Apply = true;
                        }else{
                            $count = count(Equipment::where('groupId','=',$Group->Id)->where('state', '=', '0')->get());
                            if($count>0){
                                $Group->Apply = true;
                            }else{
                                $Group->Apply=false;
...
//        DB::table('usermain')->where('NickName',null)->delete();
//        $a="http://zhiqing.isdige.cn/1504020364877.jpg";
//        dd($a);
//        return 111;
//        $Group=DB::table('MyGroup')->where('GroupName',null)->select('Id','GroupID')->get();
//        foreach($Group as $item){
//            $name=DB::table('groups')->where('Id',$item->GroupID)->select('GroupName')->first()->GroupName;
//            echo $item->GroupID.':'.$name.'<br>';
//            DB::table('MyGroup')->where('Id',$item->Id)->update([
//                'GroupName'=>$name
//            ]);
//        }


//        $GroupId=DB::table('groups')->select('Id','GroupName')->get();
////        dd($GroupId);
//        foreach($GroupId as $item){
//            echo $item->GroupName.':'.$item->Id.'<br>';
//            $uId=DB::table('MyGroup')->where('GroupID',$item->Id)->where('Type',0)->select('UserMainID')->get();
//            foreach($uId as $item1){
//                $item1->num=DB::table('MyGroup')->where('GroupID',$item->Id)->where('UserMainID',$item1->UserMainID)->where('Type',0)->count();
//                if($item1->num!=1){
//                    echo '--'.$item1->UserMainID.':'.$item1->num.'<br>';
//                    $res=DB::table('MyGroup')->where('GroupID',$item->Id)->where('UserMainID',$item1->UserMainID)->delete();
//                    if($res){
//                        echo '删除成功--';
//                        $r1=DB::table('MyGroup')->insert([
//                            'GroupID' =>$item->Id,
//                            'UserMainID' => $item1->UserMainID,
//                            'Type' => 0
//                        ]);if($r1) echo '重新插入成功--';
//                        $r2=DB::table('usermain')->where('Id',$item1->UserMainID)->decrement('GroupNum',($item1->num-1));if($r2) echo '个人加入跑团数量更新成功--';
//                        $r3=DB::table('groups')->where('Id',$item->Id)->decrement('PeopleCount',($item1->num-1));
//                        if($r3) echo '跑团人数更新成功--';
//                        if($r1&&$r2&&$r3){
//                            echo $item1->UserMainID.'修改成功'.'<br>';
//                        }else{
//                            echo $item1->UserMainID.'修改失败'.'<br>';
//                        }
//                    }
//            }
//            }
...
//        $Id=907;
//        $thisDate=DB::table('run')->where('Id',$Id)->first()->Date;
//
...
//        $Run->TotalRun=DB::table('run')->where('disRepeat',0)->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->get()->sum('Distance');
//        $Run->TotalRun=round($Run->TotalRun ,1);
//         $Run->TimeAll=DB::table('run')->where('disRepeat',0)->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->count();
//        echo $Run->TimeAll;
//        $Run->TotalPoints=DB::table('run')->where('disRepeat',0)->where('UserMainID',$Run->UserMainID)->where('Date','<=',$thisDate)->sum('Point');
